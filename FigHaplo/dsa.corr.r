...
DSAs <- paste0(unique(unlist(read.table(file = "DSAs", as.is = T))), "Rb")

Clinical$DSA <- 0

Clinical$DSA[rownames(Clinical) %in% DSAs] <- 1

#Clinical <- Clinical[Clinical$DSA == 1, ]
...
for(tt in Terms){
	...
	fit <- coxph(as.formula(paste0("Surv(GL_time, GL_event) ~", tt, " + All + Induction2 + Donor_Status2 + HLA.mm.sum + DSA")), data = Clinical)

	ResultAll <- rbind(ResultAll, cbind(summary(fit)$conf.int[, 3:4, drop = FALSE], summary(fit)$coefficients, summary(fit)$n))

}

write.table(ResultAll, file = "DSA.corr.xls", quote = F, col.names = NA, sep = "\t")
...
for(tt in Terms){
        ...
        fit <- coxph(as.formula(paste0("Surv(GL_time, GL_event) ~", tt, " + All + Induction2 + Donor_Status2 + HLA.mm.sum + DSA")), data = Clinical.CC)
        
        ResultAll <- rbind(ResultAll, cbind(summary(fit)$conf.int[, 3:4, drop = FALSE], summary(fit)$coefficients, summary(fit)$n))

}

write.table(ResultAll, file = "DSA.CC.corr.xls", quote = F, col.names = NA, sep = "\t")
