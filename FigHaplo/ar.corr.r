...
for(tt in Terms){
	...
	fit <- coxph(as.formula(paste0("Surv(GL_time, GL_event) ~", tt, " + All + Induction2 + Donor_Status2 + HLA.mm.sum + AR")), data = Clinical)

	ResultAll <- rbind(ResultAll, cbind(summary(fit)$conf.int[, 3:4, drop = FALSE], summary(fit)$coefficients, summary(fit)$n))

}

write.table(ResultAll, file = paste0(Args[1], ".corr.xls"), quote = F, col.names = NA, sep = "\t")
...
for(tt in Terms){
        ...
        fit <- coxph(as.formula(paste0("Surv(GL_time, GL_event) ~", tt, " + All + Induction2 + Donor_Status2 + HLA.mm.sum + AR")), data = Clinical.CC)
        
        ResultAll <- rbind(ResultAll, cbind(summary(fit)$conf.int[, 3:4, drop = FALSE], summary(fit)$coefficients, summary(fit)$n))

}

write.table(ResultAll, paste0(file = Args[1], ".CC.corr.xls"), quote = F, col.names = NA, sep = "\t")
